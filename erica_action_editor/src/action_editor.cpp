...
#include "erica_action_editor/action_editor.h"
...
ActionEditor::ActionEditor()
{
  ...
  //position of step and step param on terminal
  cmd_col_ = 2;
  stp7_col_ = 19 + 2;
  stp0_col_ = 25 + 2;
  stp1_col_ = 30 + 2;
  ...
  //p gain : do not change
  cwslope_col_ = 60 + 2;
  ccwslope_col_ = 61 + 2;

  //position of page information on terminal
  name_col_ = 63 + 2;
  addr_col_ = 72 + 2;
  pagenum_col_ = 75 + 2;
  pageparam_col_ = 76 + 2;
  ...
  //current page and position of cursor which user see initially
  page_idx_ = 1;
  curr_col_ = stp7_col_;
  ...
  // If last_joint_row_ is less than 9(link to exit), it means that the difference last_row_ between pause_row_ is not 1, larger than 1.
  // In this case, pause row must be specified manually.
  first_joint_row_ = 0;
...
//  }
  ...
  default_editor_script_path_ = ros::package::getPath("erica_action_editor") + "/config/editor_script.yaml";
  mirror_joint_file_path_ = ros::package::getPath("erica_action_editor") + "/config/config_mirror_joint.yaml";
