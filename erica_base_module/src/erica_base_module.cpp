...
void BaseModule::initPoseMsgCallback(const std_msgs::String::ConstPtr& msg) // GUI 에서 init pose topic을 sub 받아 실
{
  ...
  {
    init_pose_path = ros::package::getPath("erica_base_module") + "/data/init_pose.yaml";// 로스 패키지에서 YAML파일의 경로를 읽어온다.
...
void BaseModule::queueThread()
{
  ...
  /* subscribe topics */
  // for gui
  ros::Subscriber ini_pose_msg_sub = ros_node.subscribe("/heroehs/init_pose", 5, &BaseModule::initPoseMsgCallback, this);
