...
void ActionModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  std::string path = ros::package::getPath("erica_action_module") + "/data/erica_motion_4095.bin";
...
void ActionModule::queueThread()
{
  ...
  /* publisher */
  status_msg_pub_ = ros_node.advertise<robotis_controller_msgs::StatusMsg>("/heroehs/status", 0);
  done_msg_pub_ = ros_node.advertise<std_msgs::String>("/robotis/movement_done", 1);

  /* subscriber */
  ros::Subscriber action_page_sub = ros_node.subscribe("/heroehs/action/page_num", 0, &ActionModule::pageNumberCallback, this);
  ros::Subscriber start_action_sub = ros_node.subscribe("/heroehs/action/start_action", 0, &ActionModule::startActionCallback, this);

  /* ROS Service Callback Functions */
  ros::ServiceServer is_running_server = ros_node.advertiseService("/heroehs/action/is_running", &ActionModule::isRunningServiceCallback, this);
...
bool ActionModule::isRunningServiceCallback(erica_action_module_msgs::IsRunning::Request &req,
                                            erica_action_module_msgs::IsRunning::Response &res)
{
...
void ActionModule::startActionCallback(const erica_action_module_msgs::StartAction::ConstPtr& msg)
{
  if (enable_ == false)
  {
    std::string status_msg = "Action Module is not enabled";
